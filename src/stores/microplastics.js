...
export const useMicroplasticsStore = defineStore('microplastics', () => {
  ...
  async function fetch () {
    ...
    try {
      ...
      error.value = error_
      console.error('useMicroplasticsStore: fetch failed', error_)
    } finally {
      loading.value = false
    }
  }
  ...
  const colorData = computed(() => {
    ...
    const arr = Array.from(counts.values()).toSorted((a, b) => b.count - a.count).slice(0, 12)
    return {
  ...
  const sizeData = computed(() => {
    ...
    for (const it of items) {
      ...
      if (Number.isNaN(val) && it.size > 0) {
        const s = String(it.size).toLowerCase()
