...
const statsBySoil = computed(() => {
    ...
    for (const site of props.sites || []) {
        ...
        const totalMP = (Number(site.fragment_count) || 0) + (Number(site.fiber_count) || 0) +
            (Number(site.foam_count) || 0) + (Number(site.film_count) || 0) +
            (Number(site.sheets_count) || Number(site.sheet_count) || 0)
...
<template>
    <div class="boxplot-card">
        ...
        <div class="chart-slot" :style="{ minHeight: height + 'px' }">
            <div v-if="!chartSeries[0].data.length" class="state">
                No density data available
            </div>
            <ApexChartBase v-else :height="height" :options="chartOptions" :series="chartSeries" type="boxPlot" />
        </div>
    </div>
</template>
